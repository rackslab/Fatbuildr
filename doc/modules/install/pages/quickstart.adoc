...
Create `/etc/apt/sources.list.d/fatbuildr.list` with this content:
...
* Download the source tarball of https://www.gnu.org/software/hello/[GNU Hello].
  ...
  xref:usage:repo.adoc#_meta[artifact definition `meta.yml`] file located in
...
this instance, you can explore archives tasks outputs and all artifacts
...
Create APT source file `/etc/apt/sources.list.d/fatbuildr-default.list` with
this content:

[source]
----
deb [signed-by=/usr/share/keyrings/fatbuildr-default.gpg arch=amd64] http://localhost:5000/default/deb bullseye main
----
