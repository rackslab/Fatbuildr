...
All files whose name is suffixed by `.j2` is considered as a template and is
processed to generate the same file without the suffix. For example,
`deb/control.j2` generates `deb/control`.

Tokens available in templates are:
