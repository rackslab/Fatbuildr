    ...
    def _build_src(self):
        ...
        for archive in self.archives:
            if archive.is_zip:
                ...
                zip2tar(archive.path, tarball_path)
                archive.path = tarball_path

        # Extract main source archive in build place
        logger.debug(
            "Extracting main source archive %s in %s",
            self.main_archive.path,
            self.place,
        )
        main_tarball_subdir = extract_tarball(
            self.main_archive.path, self.place
        )

        # Extract other sources archives in main archive subdir
        for archive in self.archives:
            if archive.is_main(self.artifact):
                continue  # skip main archive already extracted
            target = main_tarball_subdir.joinpath(archive.id)
            logger.debug(
            ...
            if archive.has_single_toplevel():
                ...
                strip = 0
            extract_tarball(archive.path, target, strip=strip)

        # Extract supplementary tarballs that could have been generated by
        # prescript
        for archive in self.prescript_tarballs:
            logger.info(
            ...
            if archive.has_single_toplevel():
                ...
                strip = 0
            extract_tarball(archive.path, target, strip)
