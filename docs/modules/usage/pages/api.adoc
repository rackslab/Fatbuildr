...
TIP: Refer to xref:usage:fatbuildrctl.adoc[`fatbuildrctl` manpage] for more
details about this command.

CAUTION: This command only works on local Fatbuildr instance through DBus
interface. For remote authentication, first connect to the remote server (eg.
with SSH) to obtain this token and copy/paste it on the client host.
