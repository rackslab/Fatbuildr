...
*create*::
  Submit tasks to create container images. Unless `--format` filter is used, all
  images for all formats declared in instance pipelines are created. One task
  per image is submitted on server side.

*update*::
  Submit tasks to update OS packages of container images. Unless `--format`
...
*shell*::
  Submit task to run an interactive shell as root in a container image. The
  `--format` filter is required to select the image. The task is terminated when
  the shell is exited.

*env-create*::
  Submit tasks to create the build environments in the container images. Unless
  `--format` filter is used, all build environments for all formats declared in
  instance pipelines are created. One task per build environment is submitted on
  server side.

*env-update*::
  Submit tasks to update the build environments in the container images. Unless
  `--format` filter is used, all build environments for all formats declared in
  instance pipelines are updated. One task per build environment is submitted on
  server side.
...
`$ fatbuildrctl images env-update --format rpm`::
  Submit tasks to update all build environments declared in instance pipelines
  for RPM format.
--
