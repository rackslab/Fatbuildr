...
*shell*:::
  Submit a task to launch an interactive shell as root in a container image.
  Unless only one format is declared in the instance pipelines, it is necessary
  to select the image with *-f, --format* filter. The task is terminated when
  the shell is exited.
+
The *shell* subcommand accepts the following option:
...
*env-shell*:::
  Submit a task to launch an interactive shell as root in a build environment.
  The build environment must be selected with *-f, --format*,
