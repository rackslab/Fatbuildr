...
`img_create_use_sysusersd`:: Boolean to determine if _fatbuildr_ system user is
created with `sysusers.d` configuration file during image bootstrap. If not, the
user is create with raw seeds of `/etc/passwd`, `/etc/group` and `/etc/gshadow`.
